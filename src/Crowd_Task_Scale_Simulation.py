...
'''
compare markets with different scales of tasks over scale-free graph models 
'''

def TasksScaleSimulation_Homo(iterations):
    tasks = [100,200,500]
    sup_n = [x for x in range(20,201,10)]
...
def TasksScaleSimulation_Hete(times):
    tasks = [100,200,500]
    minv,maxv = 1,10
    # sup_n = [20,40,100]
    prob = 0.05
    ...
    for ts,sn in zip(tasks,sup_n): 
        ...
        data.append([cur_sc,cur_pay])
        budgs.append(bud)
    return sup_n,data,budgs

if __name__ == "__main__":
    iterations = 10
    print('*'*10 + 'Homogeneous Scenario' + '*'*10)
    x_label_t = 'the scale of providers'
    title = "crowd-iot-hm-providers-scale"
    axtitle_t = [
        'Task Scale: 100',
        'Task Scale: 200',
        'Task Scale: 500'
    ]
    xvals,data = TasksScaleSimulation_Homo(iterations)
    ...
    x_label_t = 'the scale of providers'
    title = "crowd-iot-ht-providers-scale"
    axtitle_t = [
        'Task Scale: 100',
        'Task Scale: 200',
        'Task Scale: 500'
    ]
    xvals,data,budgs = TasksScaleSimulation_Hete(iterations)
